...
11. ~~**Beden/varyant API izni:**~~ **ÇÖZÜLDÜ (2026-08-05).** Kök neden yetki değil, eksik istek parametresiydi: `product/get`'e `FetchSubProducts=true` gönderilmediğinde istek "başarılı" dönüyor ama `SubProducts` alanı hiç yer almıyordu — Faz 0 keşfi bu yüzden "erişilemiyor" sonucuna varmıştı. `FetchSubProducts=true` eklenince canlı hesapta doğrulandı: yanıt `SubProducts: [{ SubProductId, SubProductCode: "T3288|1", Property1 (Nitelik1/Renk), Property2 (Nitelik2/Beden, örn. "1"/"36"), Stock (gerçek adet), Barcode, ... }]` döndürüyor — alt ürünlerin kendi `Stock` toplamı üst seviye `Stock` alanıyla birebir tutuyor (81 = 66+15, T3288 üzerinde doğrulandı). `product/getSubProducts` vb. ayrı uçlar hâlâ AUI011 hatası veriyor ama artık gerekli değil — `product/get` tek başına yeterli. `tsoft-client.ts`'deki `mapProduct()` artık `Property2`'yi `sizeName` olarak, gerçek `Stock`'u varyant stoğu olarak okuyor; `sync.service.ts` bu etiketleri her senkronda `ProductSize`'a yazıyor (Ürün Detay ekranındaki manuel giriş artık sadece override amaçlı).
